...
def data_STATISTIC():
    ...
    tk.Label(statistic_window, text="STATISTIC!", font=25).pack()

    btn_sample = tk.Button(statistic_window, text="MEAN", bg='darkseagreen', fg='black',
                           command=data_mean)
    btn_sample.place(x=50, y=50)
    btn_sample = tk.Button(statistic_window, text="VARIANCE", bg='darkseagreen', fg='black',
                           command=data_variance)
    btn_sample.place(x=50, y=100)

    tk.Label(statistic_window, text="add_channel:").place(x=150, y=50)
    tk.Label(statistic_window, text="star sn:").place(x=150, y=80)
    tk.Label(statistic_window, text="end sn:").place(x=150, y=110)

    var1 = tk.StringVar()
    var1.set('1')
    channel_n = tk.Entry(statistic_window, textvariable=var1, width=3, relief='flat')
    channel_n.place(x=250, y=50)

    var2 = tk.StringVar()
    var2.set('0')
    sample_sn = tk.Entry(statistic_window, textvariable=var2, width=3, relief='flat')
    sample_sn.place(x=250, y=80)

    var3 = tk.StringVar()
    var3.set('1')
    sample_end = tk.Entry(statistic_window, textvariable=var3, width=3, relief='flat')
    sample_end.place(x=250, y=110)

    s_text = tk.Text(statistic_window, width=40, height=8, relief='flat')
    s_text.pack()
    s_text.place(x=300, y=300)

    statistic_window.mainloop()

def data_mean():

    datamean = np.mean(data[:, int(sfreq * int(sample_sn.get())):int(sfreq * int(sample_end.get()))], 1)
    print(data)
    print(datamean)
    plt.bar(ch_names, datamean)
    plt.show()

    #s_text = tk.Text(window, width=40, height=8, relief='flat')
    #s_text.pack()
    #s_text.place(x=700, y=450)
    #s_text.insert('insert', datamean)


def data_variance():
    datamean = np.mean(data[1, int(sfreq * int(sample_sn.get())):int(sfreq * int(sample_end.get()))])
    plt.plot(datamean)
    plt.show()
...
#--------------------------------------------------------------------------------------------------------
# fist level
btn_sample = tk.Button(window, text="1. NEW_FILE", bg='darkseagreen', fg='black', font=5, width=18, command=open_file)
btn_sample.place(x=50, y=50)
...
btn_sample = tk.Button(window, text="STATISTIC", bg='darkturquoise', fg='black', width=17, command=data_STATISTIC)
