...
def MAKE_SAMPLES():
    ...
    tk.Label(window, text="Input the width of sample:").place(x=700, y=180)
    tk.Label(window, text="Input the label of sample:").place(x=700, y=200)
    tk.Label(window, text="Input the times of sample(s):").place(x=700, y=220)

    var1 = tk.StringVar()
    var1.set('250')
    new_width = tk.Entry(window, textvariable=var1, width=3)
    new_width.place(x=890, y=180)

    var2 = tk.StringVar()
    var2.set('1')
    new_label = tk.Entry(window, textvariable=var2, width=3)
    new_label.place(x=890, y=200)

    var3 = tk.StringVar()
    var3.set('10')
    new_time = tk.Entry(window, textvariable=var3, width=3)
    new_time.place(x=890, y=220)
...
def Recognize():
    ...
    recognition_text = tk.Text(window, width=50, height=4)
    recognition_text.pack()
    recognition_text.place(x=700, y=440)
    #recognition_text.insert('insert', feature)
    recognition_text.insert('insert', np.argmax(feature, axis=1))

def RAW_INFO():
    info_text = tk.Text(window, width=40, height=8)
    info_text.pack()
...
#--------------------------------------------------------------------------------------------------------
# fist level
btn_sample = tk.Button(window, text="NEW_FILE", bg='blue', fg='yellow', font=5, width=18, command=open_file)
...
btn_sample.place(x=50, y=180)
btn_sample = tk.Button(window, text="CHOOSE_CNN", bg='blue', fg='yellow', font=5, width=18)
btn_sample.place(x=50, y=310)
btn_sample = tk.Button(window, text="RECOGNIZE", bg='green', fg='yellow', font=5, width=18, command=Recognize)
btn_sample.place(x=50, y=440)
...
btn_sample = tk.Button(window, text="MAKE_OK", bg='green', fg='yellow', width=17, command=make_OK)
btn_sample.place(x=300, y=180)
btn_sample = tk.Button(window, text="SAMPLE PLOT", bg='green', fg='yellow', width=17, command=sample_plot)
btn_sample.place(x=300, y=220)
btn_sample = tk.Button(window, text="DATA PLOT", bg='green', fg='yellow', width=17, command=data_plot)
